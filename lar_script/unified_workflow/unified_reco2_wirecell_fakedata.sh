...
do
	...
	wire-cell-imaging-lmem-celltree ./input_data_files/ChannelWireGeometry_v2.txt $wc_input_celltree $n -z1 -d1 -s2 2>&1 | tee -a ../wirecell.log
	...
	do
		...
		prod-wire-cell-matching-nusel ./input_data_files/ChannelWireGeometry_v2.txt imaging_$input2.root -z1 -d1 2>&1 | tee -a ../wirecell.log
...
for ((n=0; n<$(($wc_eventcount)); n++))
do
	wire-cell-prod-stm ./input_data_files/ChannelWireGeometry_v2.txt $wc_input_celltree $n -z1 -d0 -o0 -g2 2>&1 | tee -a ../wirecell.log
	...
	if [ -n "$wcreco2_runinfo" ]; then 
		...
		wire-cell-prod-nue ./input_data_files/ChannelWireGeometry_v2.txt $wc_input_celltree $n -z1 -d0 2>&1 | tee -a WCP_analysis.log
