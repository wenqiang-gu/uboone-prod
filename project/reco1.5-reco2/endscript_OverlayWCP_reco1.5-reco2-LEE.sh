...
input_artROOT0=`ls -t *eventweight.root | xargs | awk '{print $1}'` # find the last artROOT file
ls -t *.root | tee -a wirecell.log
echo $input_artROOT0 | tee -a wirecell.log
mv $input_artROOT0 $input_artROOT
lar -n -1 -c run_eventweight_microboone_LEE.fcl -s $input_artROOT | tee -a wirecell.log
